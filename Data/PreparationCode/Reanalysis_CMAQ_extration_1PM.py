...
def main(Path):
    ...
    input_path = os.path.join(Path , "Data/InputData/Reanalysis_CMAQ_ext/")

    # example_file = os.path.join(reanalysis_CMAQ_path, "2016/KNU_09_01/CONC.PM_RQ40i8a.KNU_09_01.20160720.nc")
...
def extract_reanalysis_CMAQ_year_month_day(year, month, day, CMAQ_root, day_dir):
    ...
    file_name = f"CONC.PM_RQ40i8a.{knu_folder}.{date_str}.nc"
...
def save_nc_as_tensor(nc_file_path, dir_save, filename):
    ...
    try:
        ...
        # Exclude non-feature variables if needed. Since we remove TFLAG, O3 is in 11-th variable (10 when starging from 0)
        exclude_vars = ["TFLAG"]
        ...
        for var in vars_to_use:
            ...
            if arr.ndim == 4:
                arr = arr[:, 0]  # remove LAY dimension by getting the first layer
