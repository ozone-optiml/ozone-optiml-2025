...
def visualize_prediction_grid_as_gif(pred, forecast_pred, true_output, true_output_no_red, device, time_str, tag=''):
    ...
    global_min = min(true_output.min(), pred.min())
    global_max = max(true_output.max(), pred.max())
    # global_min = 0
    # global_max = 0.1
    diff_pred = np.abs(pred - true_output)
    diff_forecast = np.abs(forecast_pred - true_output)
    # Setup figure
    im_true = axes[0][0].imshow(true_output[:, :, 0], cmap="viridis",
                         vmin=0, vmax=global_max,
                         origin="lower", animated=True)
    im_pred = axes[0][1].imshow(pred[:, :, 0], cmap="viridis",
                         vmin=0, vmax=global_max,
                         origin="lower", animated=True)
    im_forecast = axes[0][2].imshow(forecast_pred[:, :, 0], cmap="viridis",
                         vmin=0, vmax=global_max,
                         origin="lower", animated=True)
    im_true_no_red = axes[1][0].imshow(true_output_no_red[:, :, 0], cmap="viridis",
                         vmin=0, vmax=global_max,
                         origin="lower", animated=True)
    im_diff_pred = axes[1][1].imshow(diff_pred[:, :, 0], cmap="viridis",
                         vmin=0, vmax=global_max,
                         origin="lower", animated=True)
    im_diff_forecast = axes[1][2].imshow(diff_forecast[:, :, 0], cmap="viridis",
                         vmin=0, vmax=global_max,
                         origin="lower", animated=True)
    axes[0][0].set_title("Ground Truth (Reanalysis)")
    axes[0][1].set_title(f"Our Prediction (RMSE: {np.sqrt(np.mean((pred - true_output) ** 2)):.4f})")
    axes[0][2].set_title(f"Forecast (RMSE: {np.sqrt(np.mean((forecast_pred - true_output) ** 2)):.4f})")
    axes[1][0].set_title("Ground Truth with No Red (Reanalysis)")
    axes[1][1].set_title("Our Difference (abs)")
    axes[1][2].set_title("Forecast Difference (abs)")
    ...
    def update(frame):
        ...
        im_diff_pred.set_array(diff_pred[:, :, frame])
        im_diff_forecast.set_array(diff_forecast[:, :, frame])
        ...
        # Save frame
        frame_dir = f"Frames/{time_str}_frames"
        os.makedirs(frame_dir, exist_ok=True)
        plt.savefig(f"{frame_dir}/ozone_forecast_frame_{frame}.png", dpi=200)
        return im_true, im_pred, im_diff_pred, im_forecast, im_diff_forecast
...
if __name__=='__main__':
    ...
    val_dataset = OzoneValidationDataset(root_dir, reduction=args.reduction)
    train_dataset = OzoneTrainDataset(root_dir, reduction=args.reduction)
    test_dataset = OzoneTestDataset(root_dir, reduction=args.reduction)

    val_no_red_dataset = OzoneValidationDataset(root_dir, reduction=1)
    train_no_red_dataset = OzoneTrainDataset(root_dir, reduction=1)
    test_no_red_dataset = OzoneTestDataset(root_dir, reduction=1)
