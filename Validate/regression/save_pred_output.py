...
if __name__=='__main__':
    ...
    parser.add_argument("--model_dir", type=str, default="../../Train/checkpoints/diff_mse", help="Parent directory of the model")
    ...
    def evaluate(loader, split_name):
        output_root = os.path.join(os.path.dirname(__file__), 'pred_output')
        model_dir = os.path.join(output_root, experiment_name)
        split_dir = os.path.join(model_dir, split_name)
        os.makedirs(split_dir, exist_ok=True)

        for encoder_input, decoder_input, _, paths in tqdm(loader, desc=f"{split_name}"):
            if use_difference:
            ...
            for i in range(len(paths['output'])):
                if use_difference:
                    p = pred[i] + forecast_pred[i]
                else:
                    p = pred[i]

                date_str = os.path.basename(paths['output'][i]).split('_')[1].split('.')[0]
                np.savez_compressed(
                    os.path.join(split_dir, f"{date_str}.npz"),
