...
if __name__=='__main__':
    ...
    parser.add_argument("--diff", type=bool, default=True, help="use difference data")
    ...
    MODEL_PATH = os.path.join(SCRIPT_DIR, args.model_dir, args.model_name)
    if not MODEL_PATH.endswith(".pth"):
        MODEL_PATH += ".pth"

    # if the model doesn't exist, pick the one with the largest epoch number
    if not os.path.exists(MODEL_PATH):
        pth_files = [
            f for f in os.listdir(os.path.join(SCRIPT_DIR, args.model_dir))
            if f.startswith("epoch") and f.endswith(".pth")
        ]
        if len(pth_files) == 0:
            raise FileNotFoundError(f"No model found in {args.model_dir}")
        # extract the epoch number and pick the largest
        pth_files.sort(key=lambda x: int(x.split("epoch")[1].split("_")[0]), reverse=True)
        MODEL_PATH = os.path.join(SCRIPT_DIR, args.model_dir, pth_files[0])

    experiment_name = os.path.basename(args.model_dir.rstrip("/"))
    ...
    # --- PATCH END ---
    print(f"Using {torch.cuda.device_count()} GPU(s) for evaluation")
    ...
    use_difference = args.diff
    output_subdir = "OutputDifference" if use_difference else "Output"
    train_dataset = OzoneTrainDataset(root_dir=root_dir, reduction=args.reduction, output_subdir=output_subdir)
    validation_dataset = OzoneValidationDataset(root_dir=root_dir, reduction=args.reduction, output_subdir=output_subdir)
    test_dataset = OzoneTestDataset(root_dir=root_dir, reduction=args.reduction, output_subdir=output_subdir)
    ...
    def evaluate(loader, split_name, months=[1, 7]):
        ...
        for encoder_input, decoder_input, true_output, paths in tqdm(loader, desc=f"{split_name}"):
            ...
            encoder_input = encoder_input[keep_idx].to(device)
            decoder_input = decoder_input[keep_idx].to(device)
            true_output = true_output[keep_idx]

            with torch.no_grad():
                pred = trained_model(encoder_input, decoder_input)
                pred = pred.permute(0, 2, 1, 3).squeeze(-1).cpu().numpy()
            true_output = true_output.permute(0, 2, 1, 3).squeeze(-1).cpu().numpy()

            for i, idx in enumerate(keep_idx):
                date_str = os.path.basename(paths["output"][idx]).split("_")[1].split(".")[0]
                p = pred[i] + forecast_pred[i] if use_difference else pred[i]
                t = true_output[i] + forecast_pred[i] if use_difference else true_output[i]
