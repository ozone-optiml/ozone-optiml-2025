...
def get_args():
    ...
    parser.add_argument("--model_name", type=str, default="diff_mse/epoch96_loss0.00009_rmse0.00922.pth", help="Name of the trained model file")
...
def load_station_info(station_info_path, reduction_factor):
    ...
    station_details = stations_df.set_index('station_id').to_dict('index')
...
class OzoneO3Reanalysis(Dataset):
    """
    Dataset for training the ozone prediction model.
    Covers data from 2019 to 2021.
    """
    def __init__(self, root_dir, reduction=1, output_subdir="../Data/InputData/Reanalysis_CMAQ_ext"):
        """
    ...
    def _get_file_paths(self, start_year, end_year):
        ...
        while start_date <= end_date:
            ...
            output_path = os.path.join(self.output_subdir, year, month, day, f"{file_name}")

            if all(os.path.exists(p) for p in [output_path]):
                paths.append({
                    'output': output_path
                })
            start_date += delta
    ...
    def __getitem__(self, idx):
        ...
        # print(output[30, 30, : , 10])
        return output[:, :, :, 10]  # Return only the O3 variable
...
def main():
    ...
    test_dataset = OzoneTestDataset(args.root_dir, reduction=args.reduction, output_subdir="OutputDifference")
    test_dataset_no_red = OzoneO3Reanalysis(args.root_dir, reduction=1)
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_dataset)), desc="Evaluating Test Dataset vs Stations"):
            encoder_input, decoder_input, true_output, paths_dict = test_dataset[i] # True output is difference
            true_output_no_red = test_dataset_no_red[i] # True output without reduction and no difference
            ...
            true_output_no_red = true_output_no_red.permute(1, 0, 2).cpu().numpy()
            
            forecast_pred = encoder_input[:, :, :, -72:, 10]
            forecast_pred = (forecast_pred.squeeze(0).squeeze(-1).cpu().numpy() * std) + mean
            forecast_pred = forecast_pred.transpose(1, 0, 2)
            ...
            for day_idx, time_slice in day_slices.items():
                ...
                for station_id, details in station_details.items():
                    ...
                    row, col = details['ROW'], details['COL']
                    ...
                    reanalysis_val_no_red = np.max(true_output_no_red_ppb[row, col, time_slice])
                    ...
                    if np.isnan(observed_val):
                        continue
                    print(f"Station {station_id} on {target_date.strftime('%Y-%m-%d')}: Pred={pred_val:.2f}, Reanalysis={reanalysis_val:.2f}, Reanalysis_Nored={reanalysis_val_no_red}, Forecast={forecast_val:.2f}, Observed={observed_val:.2f}")
                    daily_results.append({
                        ...
                        'Reanalysis_no_red': reanalysis_val_no_red,
